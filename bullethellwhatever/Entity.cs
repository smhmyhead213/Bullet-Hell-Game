...
public abstract class
    Entity //wait til he finds out he has to rework the hitbox system to work with different square sizes! get a load of this guy!
{
    ...
    public bool isCollidingWithPlayer() //you need to refactor this whole thing if you wanna use rectangles
    {
        ...
        if (Math.Abs(Position.X - Main.player.Position.X) <= totalwidth &&
            Math.Abs(Position.Y - Main.player.Position.Y) <= totalwidth)
            return true;
    ...
    public virtual bool ShouldRemoveOnEdgeTouch()
    {
        return false;
    }
    ...
    {
        return false;
        //is harmful to player
    }

    public virtual bool hasDesperation()
    {
        return false;
    }
