...
namespace bullethellwhatever.UtilitySystems
{
    public static class MathsUtils
    {
        ...
        /// <param name="secondPartLength"></param>
        /// <returns></returns>
        public static float[] SolveTwoPartIK(Vector2 startPosition, Vector2 targetPosition, float firstPartLength, float secondPartLength, out Vector2 elbowPosition, int bendSign = 1)
        {
            float lengthOfLeg = firstPartLength + secondPartLength;
            ...
            elbowPosition = startPosition + Utilities.RotateVectorClockwise(direction * firstPartLength, bendSign * upperArmAngle);

            float[] output = new float[2];

            output[0] = Utilities.VectorToAngle(elbowPosition - startPosition);

            float secondPartRotation = Utilities.VectorToAngle(targetPosition - elbowPosition);
